...
fn multiple_messages_sent() {
    ...
    assert_eq!(result.ok().unwrap(), WebhookResult::new(sent_states.len() as u32, 0, 4, expected_results));
}
...
fn failures_tried_three_times() {
    ...
    assert_eq!(result.ok().unwrap(), WebhookResult::new(sent_states.len() as u32, expected_results.len() as u32, 0, expected_results));
}
